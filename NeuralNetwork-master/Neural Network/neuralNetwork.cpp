...
void neuralNetwork::initializeWeights()
{
	...
	{		
		for(int j = 0; j < nHidden; j++) 
		{
			...
			wInputHidden[i][j] = ( (( (double)(rand()%1000)+1)/1000)/10 - 0.05);//s  * 2 * rH ) - rH;
			// cout << "weight 1 = " << wInputHidden[i][j] << endl;
		}
	}
	...
	{		
		...
		{
			//set weights to random values
			wHiddenOutput[i][j] = ( (( (double)(rand()%1000)+1)/1000)/10 - 0.05);// * 2 * rO ) - rO;
			// cout << "weight 2 = " << wHiddenOutput[i][j] << endl;
		}
	}
}
/*******************************************************************
